...
compile_and_install_libpng() {
    ...
    if [ ! -d libpng-1.6.58 ]; then
        extract_archive "${LIBRARIES_PATH}/libpng-1.6.58.tar.gz"
    fi
    stage_build_directory libpng-1.6.58
...
compile_and_install_pfunit() {
    ...
    if [ ! -d pFUnit-v4.18.0 ]; then
        extract_archive "${LIBRARIES_PATH}/pFUnit-v4.18.0.tar"
    fi
    stage_build_directory pFUnit-v4.18.0
